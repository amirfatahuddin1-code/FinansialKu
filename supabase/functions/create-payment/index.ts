...
Deno.serve(async (req) => {
    ...
    try {
        ...
        const { plan_id, user_id, user_email, user_name, app_redirect_url } = await req.json()
        ...
        const midtransPayload = {
            ...
            callbacks: {
                finish: `${supabaseUrl}/functions/v1/payment-callback?redirect_uri=${encodeURIComponent(app_redirect_url || `${appUrl}/?payment_success=true`)}`,
            },
        }
        ...
        if (!midtransResponse.ok) {
            ...
            const errMsg = midtransData.error_messages?.join(', ') || JSON.stringify(midtransData)
